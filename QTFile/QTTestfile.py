...
class MyQt(QWidget):
    ...
    def CreatePageListWidget(self):
        ...
        self.PageSelectListWidget.itemActivated.connect(self.PageSelectedListWidgetOnclickListener)
    ...
    def SetAddFriendPage(self):
        ...
        ItemWidget.setProfilePicture("test.JPEG")
    ...
    def AcceptFriendOCL(self, ID):
        ...
        for i in range(self.PageContextListWidget.count()):
            if self.tmpList[i]["ID"] == ID:
                tmp = i
                break
        # self.tmpList.remove(self.tmpList[tmp])
        self.PageContextListWidget.takeItem(0)
        # self.PageContextListWidget.removeItemWidget(self.PageContextListWidget.item(tmp))
        # self.PageContextListWidget.row(self.PageContextListWidget.item(tmp))
        # self.PageContextListWidget.row
